		...
		Schema: map[string]*schema.Schema{
			...
			"min_alert_severity": {
				...
				ValidateFunc: func(value interface{}, key string) ([]string, []error) {
					switch api.SlackAlertLevel(value.(int)) {
					case api.CriticalSlackAlertLevel,
						api.HighSlackAlertLevel,
						api.MediumSlackAlertLevel,
						api.LowSlackAlertLevel,
						api.AllSlackAlertLevel:
						return nil, nil
					default:
						return nil, []error{
							fmt.Errorf(
...
func resourceLaceworkAlertChannelSlackCreate(d *schema.ResourceData, meta interface{}) error {
	var (
		...
		slack    = api.NewSlackChannelIntegration(d.Get("name").(string),
			api.SlackChannelData{
				SlackUrl:         d.Get("slack_url").(string),
				MinAlertSeverity: api.SlackAlertLevel(d.Get("min_alert_severity").(int)),
			},
		)
	)
	...
	response, err := lacework.Integrations.CreateSlackChannel(slack)
	if err != nil {
		return err
	}

	log.Println("[INFO] Verifying server response data")
	err = validateSlackChanIntegrationResponse(&response)
	if err != nil {
		return err
	}
...
func resourceLaceworkAlertChannelSlackRead(d *schema.ResourceData, meta interface{}) error {
	...
	response, err := lacework.Integrations.GetSlackChannel(d.Id())
	if err != nil {
		return err
	}
...
func resourceLaceworkAlertChannelSlackUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	response, err := lacework.Integrations.UpdateSlackChannel(slack)
	if err != nil {
		return err
	}
...
// validateSlackChanIntegrationResponse checks weather or not the server response has
// any inconsistent data, it returns a friendly error message describing the
// problem and how to report it
func validateSlackChanIntegrationResponse(response *api.SlackChanIntResponse) error {
